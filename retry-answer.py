...
def main(path_domain, path_problem, max_steps, verbose, max_errors):
    ...
    MAX_WIN_SIZE = tokenizer.model_max_length-100-sysprompt_tokens;   vprint(f"# MAX WIN SIZE= {MAX_WIN_SIZE}") # Se le añade un margen por si acaso
    context = deque([]); win_size = 0; total_deleted = 0

    ## BUCLE DE BÚSQUEDA ##
    plan = []; goal = False; n_steps = 0; n_errors = 0
    while n_steps < max_steps:
        vprint(f"\n\n== STEP {n_steps} ==" if n_errors <= 0 else f"\n\n-- step {n_steps}.{n_errors} --")

        # [!!] Se ha fallado muchas veces -> elegir acción aleatoria
        if n_errors >= max_errors:
            vprint(f"# Acción aleatoria añadida al plan")
            applicable_actions = list(simulator.get_applicable_actions(current_state))
            random_fullAction = random.choice(applicable_actions)
            chosen_action, chosen_params = random_fullAction
            current_state = simulator.apply(current_state, chosen_action, chosen_params)
            plan = repr_applicable_action(random_fullAction)

        # [i] Mostrar información sobre la ventana de contexto
        vprint(f"# window size = {win_size/MAX_WIN_SIZE * 100}%")
        ...
        for fluent, value in current_state._values.items():
            user_prompt += f"- {fluent}: {value}\n"
        user_prompt += "\n\n"
        ...
        if n_errors > 0 and n_errors < max_errors:
            user_prompt += "\nTry to answer now with the format specified in the system prompt.\n"
            user_prompt += "\nIn the previous iteration you had an error: \n "
        ...
        try:
            ...
            index = int(answer_dict["index"])
            chosen_action, chosen_params = applicable_actions[index]
            current_state = simulator.apply(current_state, chosen_action, chosen_params)

        except Exception as e:
            if isinstance(e, json.JSONDecodeError):
                vprint("[!] El JSON de respuesta no tiene un formato válido")
                error_msg = dedent("""\
                    The JSON wasn't in a valid format. Respond **EXCLUSIVELY** with a valid JSON containing:
                    1. "analysis": Here you can reason step by step, all that you want to choose the next action. Check the action history to **AVOID** loops.
                    2. "action": **ONLY THE FIRST** PDDL-formatted action
                    3. "index": The index of the actions in the users list""")

            elif isinstance(e, KeyError):
                vprint('[!] Falta el campo "index" en JSON de respuesta')
                error_msg = 'You forget the field "index" in your answer.'

            elif isinstance(e, TypeError):
                vprint('[!] El campo "index" del JSON devuelto no ha sido entero')
                error_msg = 'The field "index" of the answer MUST be a numeric variable.'
            
            elif isinstance(e, IndexError):
                vprint('[!] El campo "index" del JSON tiene que ser un entero')
                error_msg = "The choosen action isn't on the list or the number you isn't in the list. Please select a action that is contained in the previous list"

            else:
                vprint('[!] Error no identificado')
                error_msg = "The source of the error wasn't found. Try to write the answer again in the correct format."
